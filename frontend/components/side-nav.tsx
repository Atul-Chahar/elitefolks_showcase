...
export function SideNav() {
  ...
  return (
    <nav className="fixed left-0 top-0 z-50 h-screen w-16 md:w-20 hidden md:flex flex-col justify-center bg-transparent pointer-events-none">
      <div className="flex flex-col gap-6 px-4 pointer-events-auto">
        {navItems.map(({ id, label }) => (
          <button suppressHydrationWarning key={id} onClick={() => scrollToSection(id)} className="group relative flex items-center gap-3">
            <span
              className={cn(
